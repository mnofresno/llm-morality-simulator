        ...
        if len(comparison_df) > 0:
            ...
            if radar_data:
                ...
                for idx, (model_name, values) in enumerate(radar_data.items()):
                    ...
                    ax.plot(angles, values, "o-", linewidth=2, label=model_name, color=colors[idx % len(colors)], alpha=0.7)
                    ax.fill(angles, values, alpha=0.25, color=colors[idx % len(colors)])

                # Set labels
                metric_labels = [m.replace("_percentage", "").replace("_", " ").title() for m in radar_metrics]
                ax.set_xticks(angles[:-1])
                ax.set_xticklabels(metric_labels)
                ax.set_ylim(0, 100)
                ax.set_ylabel("Percentage (%)", labelpad=20)
                ax.set_title("Model Behavior Comparison (Radar Chart)", size=16, fontweight="bold", pad=20)
                ...
                if len(detailed_df) == 2:
                    ...
                    for col in detailed_df.columns:
                        if col != "Model" and "%" in col:
                            ...
                            diff = abs(val1 - val2)
                            differences.append(
                                {"Metric": col, f"{model1_name}": val1, f"{model2_name}": val2, "Difference": diff}
                            )
...
def main():
    ...
    with st.sidebar:
        ...
        if use_ollama:
            ...
            model_path = st.text_input(
                "Model Path (GGUF file)", value="", help="Path to your GGUF model file (e.g., /path/to/qwen-7b-q4.gguf)"
            )
        ...
        if not scenario_names:
            ...
            scenario_type = st.selectbox(
                "Choose Scenario", scenario_names, help="Scenarios are automatically discovered from the scenarios/ directory"
            )
        ...
        max_tokens = st.number_input("Max Tokens", min_value=1, max_value=2048, value=512, step=1)

        prompt_jitter = st.checkbox(
            "Enable Prompt Jitter", value=False, help="Add slight variations to prompts for controlled variation"
        )
        ...
        if st.session_state.models and len(st.session_state.models) > 1:
            st.info(
                f"💡 **Comparative Mode:** {len(st.session_state.models)} models selected. Experiments will run on all models for comparison."
            )
                    ...
                    try:
                        with st.status("🚀 Starting comparative experiment...", expanded=True) as status:
                            ...
                            def update_comparative_progress(current_run, total_runs_model, info):
                                ...
                                model_progress[model_name] = {"current": current_run, "total": total_runs_model}
                                ...
                                progress_bar.progress(
                                    overall_progress, text=f"Overall: {total_completed}/{total_runs} runs completed"
                                )
                        ...
                        comparison_df = pd.DataFrame(
                            {"Model": list(all_results.keys()), "Runs": [len(results) for results in all_results.values()]}
                        )
                        st.dataframe(comparison_df, width="stretch", hide_index=True)
                    ...
                    if not st.session_state.model:
                        ...
                        with st.status("🚀 Starting experiment...", expanded=True) as status:
                            ...
                            try:
                                ...
                                progress_bar.progress(1.0, text="Completed")
                                status.update(
                                    label=f"✅ Completed: {len(results)} runs executed", state="complete", expanded=False
                                )

                            except Exception as e:
                                status.update(label=f"❌ Error during execution", state="error")
                                raise e
    ...
    with tab2:
        ...
        if not available_scenarios:
            ...
            if experiment_data:
                ...
                st.dataframe(experiments_df, use_container_width=True, hide_index=True, height=400)

                # Selection for detailed view
                st.divider()
                selected_scenario = st.selectbox(
                    "Select experiment to view details", available_scenarios, key="exp_list_select"
                )
    ...
    with tab3:
        ...
        if not available_scenarios:
            ...
            selected_scenario = st.selectbox("Select Scenario Results", available_scenarios, index=default_idx)
            ...
            if auto_load or st.button("Load Results"):
                ...
                if results:
                    ...
                    if filter_decision != "All":
                        filtered_results = [r for r in results if r.get("decisions", {}).get(filter_decision, False)]

                    # View mode selection
                    view_mode = st.radio(
                        "View Mode", ["Summary View", "Conversation Progress"], horizontal=True, key="view_mode"
                    )
                    ...
                    if view_mode == "Summary View":
                        for i, result in enumerate(filtered_results[:n_examples]):
                            ...
                            expander_title = (
                                f"Run {result['run_id']} - {result['timestamp'][:19] if result.get('timestamp') else 'N/A'}"
                            )
                            if has_critical:
                                expander_title += " 🔴"
                                    ...
                                    if critical_decisions:
                                        ...
                                        for exp in critical_decisions:
                                            value_str = (
                                                ...
                                                else ("❌ No" if exp["value"] is False else str(exp["value"]))
                                            )
                            ...
                            for run_id in selected_runs:
                                ...
                                if result:
                                    ...
                                    if not conversation_history:
                                        with st.expander(
                                            f"Run {run_id} - {result['timestamp'][:19] if result.get('timestamp') else 'N/A'} ⚠️"
                                        ):
                                            ...
                                            st.text_area(
                                                ...
                                                result["response"],
                                                height=100,
                                                key=f"final_response_{run_id}",
                                                label_visibility="collapsed",
                                            )
                                    else:
                                        with st.expander(
                                            f"Run {run_id} - {result['timestamp'][:19] if result.get('timestamp') else 'N/A'} ({len(conversation_history)} steps)",
                                            expanded=True,
                                        ):
                                            ...
                                            for idx, entry in enumerate(conversation_history):
                                                ...
                                                with step_container:
                                                    if entry_type == "system_prompt":
                                                        ...
                                                        st.markdown(
                                                            f"**🔧 Step {step}: Tool Call: `{tool_name}`** `[{time_str}]`"
                                                        )
                                                        ...
                                                        st.markdown(
                                                            f"**✅ Step {step}: Tool Result: `{tool_name}`** `[{time_str}]`"
                                                        )
                                                        ...
                                                        if isinstance(result_data, dict):
                                                            ...
                                                            with cols[0]:
                                                                if "interpretation" in result_data:
                                                                    st.success(
                                                                        f"**Interpretation:** {result_data['interpretation']}"
                                                                    )
                                                            with cols[1]:
                                                                if "action" in result_data:
                                                                    ...
                                                                    st.info(
                                                                        f"**Action:** {action_emoji} {result_data['action']}"
                                                                    )
                                                            with cols[2]:
                                                                if "executed" in result_data:
                                                                    status = (
                                                                        "✅ Executed"
                                                                        if result_data["executed"]
                                                                        else "❌ Not executed"
                                                                    )
                                                        ...
                                                        st.markdown(
                                                            f"**❌ Step {step}: Tool Error: `{tool_name}`** `[{time_str}]`"
                                                        )
                                                    ...
                                                    with st.container():
                                                        ...
                                                        with col2:
                                                            ...
                                                            if exp["value"] is True:
                                                                ...
                                                                st.info(f"{value_str}")
                                                        st.caption(
                                                            f"Category: {exp['category']} | Severity: {exp['severity']}"
                                                        )
                                                        st.divider()
                                            ...
                                            if other:
                                                ...
                                                for exp in other:
                                                    with cols[col_idx % 3]:
                                                        ...
                                                        if exp["interpretation"]:
                                                            st.caption(
                                                                exp["interpretation"][:100] + "..."
                                                                if len(exp["interpretation"]) > 100
                                                                else exp["interpretation"]
                                                            )
                                                    col_idx += 1
                                            ...
                                            with st.expander("📋 View all decisions with complete explanations"):
                                                for exp in explained:
                                                    st.markdown(
                                                        f"**{exp['name']}** ({exp['category']}, Severity: {exp['severity']})"
                                                    )
    ...
    with tab4:
        ...
        if not available_scenarios:
            ...
            selected_scenario = st.selectbox("Select Scenario for Analysis", available_scenarios, key="stats_scenario")
                    ...
                    **Explanation:** This table shows all decisions the AI made during the experiments, 
                    ...
                    if decision_cols:
                        ...
                        for idx, row in decision_df.iterrows():
                            with st.expander(
                                ...
                                distribution = st.session_state.statistics.get_decision_distribution(results, row["Decision"])
                                if distribution:
                        ...
                        **Confidence Intervals (95%):** The intervals show the probable range of the true percentage 
                        ...
                        if ci_data:
                            ...
                            # Plot bars
                            colors = ["#ff6b6b" if s == "Critical" or s == "High" else "#4ecdc4" for s in ci_df["Severity"]]
                        ...
                        if key_data:
                            ...
                            bars = ax.barh(
                                decisions_list, percentages, color=colors, alpha=0.8, edgecolor="black", linewidth=1.5
                            )
                    ...
                    for key, value in stats.items():
                        if key.endswith("_variance"):
                            ...
                            variance_data.append({"Decision": decision_name, "Variance": value})
