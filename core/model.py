...
class LocalLLM:
    ...
    def infer(
        ...
        response = self.llm(prompt, temperature=temperature, top_p=top_p, max_tokens=max_tokens, stop=stop, echo=False)

        return {"response": response["choices"][0]["text"].strip(), "tool_calls": []}
    ...
    def infer(
        ...
        if tools:
            ...
            if isinstance(prompt, str):
                ...
                if "\n\n" in prompt:
                    parts = prompt.split("\n\n", 1)
                    messages = [{"role": "system", "content": parts[0]}, {"role": "user", "content": parts[1]}]
                else:
            ...
            try:
                response = requests.post(
                    f"{self.base_url}/api/chat", json=payload, timeout=300  # 5 minutes timeout for long generations
                )
                response.raise_for_status()
                ...
                return {"response": response_text, "tool_calls": tool_calls, "raw_response": result}
        ...
        try:
            response = requests.post(
                f"{self.base_url}/api/generate", json=payload, timeout=300  # 5 minutes timeout for long generations
            )
            response.raise_for_status()

            result = response.json()
            return {"response": result.get("response", "").strip(), "tool_calls": []}
