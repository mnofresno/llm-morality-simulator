...
class ResultsStorage:
    ...
    def _load_sqlite(self, scenario_name: Optional[str], experiment_id: Optional[str], model_name: Optional[str]) -> List[Dict[str, Any]]:
    ...
    def _load_duckdb(self, scenario_name: Optional[str], experiment_id: Optional[str], model_name: Optional[str]) -> List[Dict[str, Any]]:
    ...
    def list_experiments(self) -> List[str]:
        ...
        if self.backend == StorageBackend.JSONL:
            ...
            if self.backend == StorageBackend.SQLITE:
                conn = sqlite3.connect(str(self.db_path))
                cursor = conn.cursor()
                cursor.execute("SELECT DISTINCT experiment_id FROM results WHERE experiment_id IS NOT NULL ORDER BY experiment_id")
                experiments = [row[0] for row in cursor.fetchall()]
                conn.close()
            else:  # DuckDB
                conn = duckdb.connect(str(self.db_path))
                result = conn.execute("SELECT DISTINCT experiment_id FROM results WHERE experiment_id IS NOT NULL ORDER BY experiment_id").fetchall()
    ...
    def list_models(self) -> List[str]:
        ...
        if self.backend == StorageBackend.JSONL:
            ...
            if self.backend == StorageBackend.SQLITE:
                ...
                conn = duckdb.connect(str(self.db_path))
                result = conn.execute("SELECT DISTINCT model_name FROM results WHERE model_name IS NOT NULL ORDER BY model_name").fetchall()
