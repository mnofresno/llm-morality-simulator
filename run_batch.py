...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run LLM behavior experiments in batch mode")

    parser.add_argument(
        "--model", type=str, required=True, help="Path to GGUF model file or Ollama model name (e.g., 'qwen3:14b')"
    )

    parser.add_argument("--use-ollama", action="store_true", help="Use Ollama instead of direct GGUF file")
    ...
    parser.add_argument("--n-runs", type=int, default=100, help="Number of runs to execute")
    ...
    parser.add_argument("--prompt-jitter", action="store_true", help="Enable prompt jitter for controlled variation")

    parser.add_argument(
        "--jitter-probability", type=float, default=0.1, help="Probability of applying prompt jitter (if enabled)"
    )

    parser.add_argument("--results-dir", type=str, default="results", help="Directory to save results")

    parser.add_argument("--show-stats", action="store_true", help="Show statistics after running experiment")
    ...
    try:
        ...
        for run_id in tqdm(range(args.n_runs), desc="Experiment Progress", unit="run", ncols=80):
            ...
            try:
                inference_result = model.infer(
                    prompt=full_prompt, temperature=args.temperature, top_p=args.top_p, max_tokens=args.max_tokens
                )
            ...
            if (run_id + 1) % max(1, args.n_runs // 20) == 0 or run_id == args.n_runs - 1:
                ...
                print(
                    f"{run_id+1:<6} {100*(run_id+1)/args.n_runs:>5.1f}%     "
        ...
        if args.show_conversation_after or args.show_conversation is not None:
            ...
            runner.show_experiment_progress(results, run_ids=run_ids_to_show, show_timestamps=True, max_width=80)
