...
def test_statistics_get_example_responses_no_matches():
    ...
    mock_results = [
        {"run_id": i, "scenario": "test", "response": f"Response {i}", "decisions": {"harm_decision": False}, "metadata": {}}
        for i in range(5)
    ]
...
def test_statistics_get_decision_distribution_with_none():
    ...
    mock_results = [
        {"run_id": i, "decisions": {"test_decision": None if i % 2 == 0 else True}, "metadata": {}} for i in range(5)
    ]
